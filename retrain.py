...
subfolders, each full of images for each label. The example folder
...
to validate that you have gathered good training data, but if you want to
deploy on resource-limited platforms, you can try the
 `--architecture` flag with a Mobilenet model. For example:
...
'0.25' to control the size, and the second controls the input image size,
either '224', '192', '160', or '128', with smaller sizes running faster. See
...
def create_image_lists(image_dir, testing_percentage, validation_percentage):
    ...
    Returns:
      A dictionary containing an entry for each label subfolder, with
      images split
    ...
    for sub_dir in sub_dirs:
        ...
        if len(file_list) < 20:
            tf.logging.warning(
                'WARNING: Folder has less than 20 ' /
                'images, which may cause issues.')
        elif len(file_list) > MAX_NUM_IMAGES_PER_CLASS:
            tf.logging.warning(
                'WARNING: Folder {} has more than {} images. Some images will '
                'never be selected.'.format(
        ...
        for file_name in file_list:
            ...
            # has a way of grouping photos that are close variations
            # of each other. For example this is used in the plant
            # disease data set to group multiple pictures of the same leaf.
            hash_name = re.sub(r'_nohash_.*$', '', file_name)
            # This looks a bit magical, but we need to decide
            # whether this file should go into the training, testing,
            #  or validation sets, and we want to keep existing files
            # in the same set even if more files are subsequently added.
            # To do that, we need a stable way of deciding based
            # on just the file name itself, so we do a hash of that and
            # then use that to generate a probability value that we
            # use to assign it.
...
def create_model_graph(model_info):
    ...
    Args:
      model_info: Dictionary containing information about
...
def get_or_create_bottleneck(sess, image_lists, label_name, index, image_dir,
    ...
    with open(bottleneck_path, 'r') as bottleneck_file:
        bottleneck_string = bottleneck_file.read()
        # Allow exceptions to propagate here, since
        # they shouldn't happen after a
        # fresh creation
    bottleneck_values = [float(x) for x in bottleneck_string.split(',')]
    return bottleneck_values
...
def get_random_cached_bottlenecks(sess, image_lists, how_many, category,
    ...
    if how_many >= 0:
        ...
        for unused_i in range(how_many):
            ...
            im_index = random.randrange(MAX_NUM_IMAGES_PER_CLASS + 1)
            image_name = get_image_path(
                image_lists, label_name, im_index,
                image_dir, category)
        ...
        for label_index, label_name in enumerate(image_lists.keys()):
            for im_index, image_name in enumerate(image_lists[label_name][category]):
                    image_name = get_image_path(
                        image_lists, label_name,
                        image_index, image_dir, category)
            bottleneck = get_or_create_bottleneck(
                sess, image_lists, label_name, im_index, image_dir,
                category, bottleneck_dir, jpeg_data_tensor,
                decoded_image_tensor, resized_input_tensor,
                bottleneck_tensor, architecture)
            ground_truth = np.zeros(class_count, dtype=np.float32)
...
def get_random_distorted_bottlenecks(
    ...
    bottleneck values. Instead we find random images for the
    requested category, run them through the distortion graph,
    and then the full graph to get the
...
def add_input_distortions(flip_left_right, random_crop, random_scale,
    ...
    input image. If it's zero, then the box is the same size as the input
    and no cropping is performed. If the value is 50%, then the
    box will be half the width and height of the input.
    ...
    centered and its size varies randomly within the given range.
    For example if
    the scale percentage is zero, then the bounding box is the same size as the
    input and no scaling is applied. If it's 50%, then the bounding
    box will be in
...
def add_final_training_ops(class_count, final_tensor_name, bottleneck_tensor,
    ...
    We need to retrain the top layer to identify our new classes,
    so this function adds the right operations to the graph,
    along with some variables to hold the weights, and then sets
    up all the gradients for the backward pass.
    ...
    Returns:
      The tensors for the training and cross entropy results,
      and tensors for the bottleneck input and ground truth input.
  """
...
def create_model_info(architecture):
    ...
    retrained using transfer learning, and this function translates from
    the name
    ...
    if (size_string != '224' and size_string != '192' and
        ...
        if len(parts) == 3:
            ...
            if parts[3] != 'quantized':
                tf.logging.error(
                    "Couldn't understand architecture" /
                    "suffix '%s' for '%s'", parts[3],
                    architecture)
            return None
        is_quantized = True
...
def main(_):
    ...
    with tf.Session(graph=graph) as sess:
        ...
        if do_distort_images:
            # We will be applying distortions, so setup the
